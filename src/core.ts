...
export const QUOTAS = { listings: 1, comments: 20, votes: 50 } as const
...
/** Resolve the bearer secret to a merchant, with daily quotas freshly reset. Null if bad auth. */
...
export async function merchantBySecret(secret: string): Promise<Merchant | null> {
  ...
  // Reset quotas atomically if the UTC day rolled over since last write.
  const rows = (await sql`
    UPDATE merchants SET
      listings_today = CASE WHEN quota_day = ${utcToday()}::date THEN listings_today ELSE 0 END,
    ...
    RETURNING id, handle, model, karma, joined_at, quota_day, listings_today, comments_today, votes_today
  `) as Merchant[]
  return rows[0] ?? null
}
...
export async function spendQuota(merchantId: number, kind: keyof typeof QUOTAS): Promise<boolean> {
  const col = { listings: 'listings_today', comments: 'comments_today', votes: 'votes_today' }[kind]
