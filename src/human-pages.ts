...
const CITY_BRIDGE_BODY = `<main id="main-content" class="guide-main">
  ...
  <section class="guide-section" aria-labelledby="seller-bridge-title">
    ...
    <div class="callout">
      ...
      <p><strong>Draft-value contract.</strong> After trimming, <code>title</code> is ${MARKET_LIMITS.listing.titleMinChars}-${MARKET_LIMITS.listing.titleMaxChars} characters, <code>description</code> is ${MARKET_LIMITS.listing.descriptionMinChars}-${MARKET_LIMITS.listing.descriptionMaxChars} characters, and <code>preview</code> is at most ${MARKET_LIMITS.listing.previewMaxChars} characters; these text limits count UTF-16 code units. <code>price_usdc</code> must be greater than 0 and at most ${MARKET_LIMITS.listing.priceMaxUsdc.toLocaleString('en-US')}; the market rounds it to ${MARKET_LIMITS.listing.priceDecimals} decimal places. <code>seller_wallet</code> is <code>0x</code> plus 40 hex characters. <code>thing_id</code> is a positive integer. To avoid silent loss, send <code>tags</code> as at most ${MARKET_LIMITS.listing.tagsMaxCount} values of at most ${MARKET_LIMITS.listing.tagMaxChars} UTF-16 code units. The market lowercases and trims tags, removes values that are empty or duplicate at that point, truncates each remaining value to ${MARKET_LIMITS.listing.tagMaxChars} UTF-16 code units, and keeps the first ${MARKET_LIMITS.listing.tagsMaxCount}.</p>
