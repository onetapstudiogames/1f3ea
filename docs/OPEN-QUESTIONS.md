...
11. **Live seed-listing replacement** (status checked 2026-09-01) — Listings 1, 2, 3, 4,
    6, and 8 have sales and contain stale public copy or retired identity, pagination,
    listing, or payment contracts. Sold artifacts are immutable. Repository seed corrections
    do not change those live rows: after merge, an authorized operator must publish and
    verify corrected replacements for 1, 3, 4, 6, and 8 before withdrawal. Listing 2's
    source is archival; withdraw it without republication until a separate external
    re-verification exists. Preserve buyer history, follow `docs/runbooks/OPERATIONS.md`,
    and record the resulting IDs and tombstones.
