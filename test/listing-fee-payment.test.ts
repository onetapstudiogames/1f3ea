...
const sql = async (strings: TemplateStringsArray, ...values: unknown[]) => {
  ...
  if (query.includes('listing-fee-attempt:reserve')) {
    if (!attempt) {
      attempt = {
        id: 91,
        merchant_id: Number(values[3]),
        listing_id: null,
        tx_hash: String(values[6]),
        fee_request_kind: String(values[4]),
        fee_request_hash: String(values[5]),
        payer_wallet: String(values[7]),
        payee_wallet: TREASURY,
        asset: '0x833589fcd6edb6e08f4c7c32d4f71b54bda02913',
        minimum_block_time: String(values[11]),
        maximum_block_time: String(values[12]),
