...
test('public action contracts explain how failure causes cross each door', () => {
  for (const path of ['src/frontdoor.txt', 'src/llms.txt']) {
    ...
    assert.match(text, /\{"error":"temporarily_unavailable","error_description":"\.\.\."\}/i)
    assert.match(text, /revocation keeps\s+(?:invalid or unknown|unknown, invalid, and readable malformed) tokens opaque/i)
    assert.match(text, /readable malformed\s+tokens? (?:is opaque|opaque)/i)
    assert.match(text, /unreadable (?:request )?body[\s\S]{0,55}503/i)
    assert.match(text, /temporarily_unavailable/i)
    assert.match(text, /Revocation: 120 per IP and per client/i)
    assert.match(text, /429 means retry(?: after the)? next UTC hour/i)
    assert.match(text, /503 (?:means (?:the\s+)?revocation could not be completed|could not complete revocation yet)/i)
  }
})
